...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //If internet connected, execute the async tasks to download ths stocks' data
        if(connected()){
            for(int i=0; i < dbStocks.size(); i++){
                new FinancialDataDownloader(this, -1).execute(new String[] {dbStocks.get(i)[0], dbStocks.get(i)[1]});
            }
    ...
    public void doRefresh(){
        ...
        if(connected()){
            ...
            for(int i=0; i < stockList.size(); i++){
                new FinancialDataDownloader(this, i).execute(new String[] {stockList.get(i).getSymbol(), stockList.get(i).getCompany()});
            }
            ...
            if(items.size() > 0){
                ...
                ADB.setItems(listPrompt, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        new FinancialDataDownloader(MainActivity.this, -1).execute(new String[]{items.get(which), stockMap.get(items.get(which))});
